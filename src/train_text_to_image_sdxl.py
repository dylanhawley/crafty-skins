...
def main(args):
    ...
    def preprocess_train(examples):
        ...
        for image in images:
            ...
            crop_top_left = (0, 0)  # No crop offset since we're not cropping
    ...
    with accelerator.main_process_first():
        ...
        # Remove all original columns from train_dataset_with_vae except the new 'model_input' column
        # to avoid duplicate columns when concatenating
        vae_columns_to_keep = ["model_input"]
        vae_columns_to_remove = [col for col in train_dataset_with_vae.column_names if col not in vae_columns_to_keep]
        
        precomputed_dataset = concatenate_datasets(
            [train_dataset_with_embeddings, train_dataset_with_vae.remove_columns(vae_columns_to_remove)], axis=1
        )
