...
class CompositeImageDataset(Dataset):
    ...
    def __init__(
        ...
        for img_path in self.data_dir.rglob('*'):
            if img_path.suffix.lower() in self.image_extensions and not img_path.name.startswith('._'):
...
class SDXLLoRATrainer:
    ...
    def _load_models(self):
        ...
        # Disable xformers for all models
        self._disable_xformers()
        
    def _disable_xformers(self):
        """Disable xformers memory efficient attention for all models."""
        logger.info("Disabling xformers memory efficient attention")
        ...
        # Disable for VAE
        if hasattr(self.vae, "disable_xformers_memory_efficient_attention"):
            self.vae.disable_xformers_memory_efficient_attention()
        
        # Disable for text encoders
        if hasattr(self.text_encoder_1, "disable_xformers_memory_efficient_attention"):
            self.text_encoder_1.disable_xformers_memory_efficient_attention()
        
        if hasattr(self.text_encoder_2, "disable_xformers_memory_efficient_attention"):
            self.text_encoder_2.disable_xformers_memory_efficient_attention()
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                with self.accelerator.accumulate(self.unet):
                    ...
                    with torch.no_grad():
                        # Convert images to float16 to match VAE dtype
                        images_fp16 = images.to(dtype=torch.float16)
                        masked_images_fp16 = masked_images.to(dtype=torch.float16)
                        
                        latents = self.vae.encode(images_fp16).latent_dist.sample()
                        latents = latents * self.vae.config.scaling_factor
                        
                        masked_latents = self.vae.encode(masked_images_fp16).latent_dist.sample()
...
def main():
    ...
    parser.add_argument("--model_id", type=str, default="diffusers/stable-diffusion-xl-1.0-inpainting-0.1", help="SDXL model ID")
